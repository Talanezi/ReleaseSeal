...
| Local caption draft | Timed fake local-ASR segments over an exact video | Ordered UTF-8 SRT; machine provenance; safe filename; no deterministic contract promotion | `generated_captions.py`, caption API | `test_generated_captions.py`, `test_api.py::test_local_caption_generation_reuses_machine_draft_for_advisory_evidence`, frontend caption-generation test | Downloadable draft and one-pass M31 reuse |
| Authentic thumbnail text fallback | Public-domain USGS Yellowstone thumbnail | Consolidated title-artwork regions after primary abstention | `thumbnail_assurance.py` segmentation fallback | `test_thumbnail_assurance.py::test_authentic_usgs_title_uses_conservative_segmentation_fallback` | Text regions found; textless/busy controls still abstain |
