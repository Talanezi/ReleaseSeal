...
from releaseseal.models import FindingSeverity
from releaseseal.thumbnail_assurance import ThumbnailAssurancePolicy
...
class TranscriptionConfig(BaseModel):
    ...
    maximum_generated_caption_cues: int = Field(default=3000, ge=1, le=5000)
    maximum_generated_caption_characters: int = Field(default=100_000, ge=1_000, le=500_000)
...
def load_config(path: str | Path) -> PreflightConfig:
    ...
    try:
        ...
        raise ConfigurationError(
            "ReleaseSeal configuration is invalid.", errors=errors
        ) from exc
